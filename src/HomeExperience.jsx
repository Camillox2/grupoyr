...
import { BrandLogo, DeveloperCredit } from './App.jsx'
...
export default function HomeExperience({onContact,onLegal,onGuide}) {
  ...
  return <div className="yr3">
    ...
    <footer className="yr3-footer"><div className="yr3-width yr3-footer-grid"><div className="footer-brand-pair"><BrandLogo/><DeveloperCredit /></div><div><strong>Explore</strong><a href="#produtos">Equipamentos</a><a href="/comprar-ou-alugar">Guia de compra e locação</a><a href="/blog">Blog YR</a><a href="#porque-yr">Sobre a YR</a></div><div><strong>Converse</strong><a href={siteConfig.whatsappLink || `https://wa.me/${siteConfig.whatsapp}`} target="_blank" rel="noreferrer">WhatsApp: {siteConfig.whatsappLabel}</a><a href={`mailto:${siteConfig.email}`}>{siteConfig.email}</a></div><div><strong>Informações</strong><button onClick={()=>onLegal('privacy')}>Privacidade</button><button onClick={()=>onLegal('terms')}>Termos de uso</button></div></div><div className="yr3-width yr3-footer-bottom"><span>© 2026 Grupo YR Hospitalar.</span><span>Modelos e disponibilidade são confirmados na cotação.</span></div></footer>
  </div>
}
