...
export const BlogManagerView: React.FC = () => {
  ...
  return <div className="space-y-5 pb-12">
    <header className="flex flex-col justify-between gap-4 rounded-2xl border border-blue-100 bg-[#f8fbff] p-5 shadow-sm lg:flex-row lg:items-center">
      <div><h2 className="flex items-center gap-2 text-base font-bold text-slate-900"><BookOpen className="h-5 w-5 text-[#123b63]" />Central editorial do Blog YR</h2><p className="mt-1 text-xs text-slate-500">Artigos persistidos no CRM, com capa, galeria, SEO e fluxo de rascunho.</p></div>
      <button onClick={newArticle} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#123b63] px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-[#0b2d4c]"><Plus className="h-4 w-4" />Novo artigo</button>
    </header>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <section className="overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-blue-100 bg-[#f8fbff] px-5 py-4"><div><h3 className="text-sm font-bold text-slate-900">{selected ? 'Editar artigo' : 'Novo artigo'}</h3><p className="mt-0.5 text-[11px] text-slate-500">JPG, PNG e WebP de até 5 MB.</p></div><span className={`rounded-lg px-2.5 py-1 text-[10px] font-bold ${draft.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-[#123b63]'}`}>{statusText[draft.status]}</span></div>
        <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="space-y-4">
            ...
            <div><div className="mb-1.5 flex items-center justify-between"><span className="text-xs font-bold text-slate-700">Imagem de capa</span>{draft.coverImage && <button onClick={() => setDraft((item) => ({ ...item, coverImage: null }))} className="text-[11px] font-semibold text-rose-600">Remover</button>}</div>
              ...
              {draft.coverImage && <label className="mt-2 inline-flex cursor-pointer items-center gap-1.5 text-[11px] font-bold text-[#123b63]"><Upload className="h-3.5 w-3.5" />Trocar imagem<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => addCover(e.target.files)} className="hidden" /></label>}
            ...
            <div><div className="mb-1.5 flex items-center justify-between"><span className="text-xs font-bold text-slate-700">Galeria do artigo</span><label className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-[11px] font-bold text-[#123b63]"><ImagePlus className="h-3.5 w-3.5" />Adicionar fotos<input type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={(e) => addGallery(e.target.files)} className="hidden" /></label></div>
